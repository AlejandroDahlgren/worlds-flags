...
const CountryDetails = () => {
  ...
  return (
    <div className="country-container">
      ...
      <img className="flags" src={countryData.flags.png} alt={`Flag of ${countryData.name.common}`} />
        <h2>{countryName.official}</h2>
        <p>Population: {population || 'N/A'}</p>
        <p>Region: {region || 'N/A'}</p>
        <p>Capital: {capital || 'N/A'}</p>
        <p>Native name: {countryName.nativeName ? countryName.nativeName.common : 'N/A'}</p>
        <p>Currencies: {currencies ? currencies[Object.keys(currencies)[0]].name : 'N/A'}</p>
        <p>Languages: {languages ? Object.values(languages).join(', ') : 'N/A'}</p>
      </div>
    </div>
  );
};
